...
11. **Checkpoint/record** — append decisions/results, status, verification receipts and a fresh RESUME snapshot that names the exact next plan task/action; preserve unresolved hazards/deviations. Before a long/manual/device/external gate, write a **durable recovery checkpoint** with exact head/tree, dirty/user-owned state, fresh evidence already accepted, remaining criteria, external/device state and next action. For visual/device-only evidence, prefer a fresh bounded `flow-evidence-verifier` (`@vision`) and keep the architect as the acceptance judge.
12. **Rotate context when earned** — after a durable phase boundary such as accepted planning → execution supervision or coherent implementation → device/integration verification, prefer a fresh controller session when the old Main transcript no longer contributes material judgment. The RESUME pointer must make this a terse restart, not a reconstruction exercise or approval gate.
