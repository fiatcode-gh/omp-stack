...
When Main has useful independent controller work, do it while children run. When the next meaningful action depends on a child and no useful independent work remains, native `hub wait` is valid. In OMP, `hub wait` is interruptible by user steering: it parks Main's autonomous execution but does not prevent the user from prompting, and ordinary child job results may also self-deliver.

Choose waiting by dependency, not ritual. Prefer one appropriately scoped wait when Main genuinely needs the result next; otherwise yield and let completion self-deliver. Do not turn waiting into progress polling: repeated short `hub wait` calls or repeated `hub jobs` snapshots without new work are orchestration noise. `hub send` with `await: true` or a peer-filtered reply wait remains useful for targeted live request/response. A completed non-isolated owner revived by `hub send` is a live agent, not a new task job: do not wait on its old task job id after revival.

The `flow-evidence-guard` runtime extension deliberately does not intercept Agent Hub waits. OMP owns wait and steering mechanics; this extension only backstops the evidence-verifier capsule manifest.
...
If a production-, asset-, or build-affecting correction happens after the coherent result already crossed acceptance — including a defect discovered during device/manual/external evidence — re-enter the stability barrier before integration. Run scoped independent acceptance/closure on the changed final tree, then map prior evidence capsules to the changed dependency surface: rerun affected capsules and reuse unaffected capsules only with an explicit freshness rationale. A downstream evidence failure that forces such a mutation is itself a concrete unresolved acceptance risk, so it can justify an additional scoped closure even when the normal one-closure ceiling was already reached.
...
The evidence-capsule runtime preflight rejects verifier dispatches missing these markers; the verifier independently rejects a manifest that still packs separable scene families. Split by **acceptance modality/operator** as well as scene/state: shared setup, save, screen, or device state does not by itself make evidence inseparable. Evidence with different acceptance modalities or operators is independently restartable by default. Automated visual/device capture and acceptance that requires physical human-operated interaction, including real assistive-technology touch exploration, belong in separate capsules unless completing one genuinely requires the other to remain in the same verifier session. A capsule must not use synthetic input, ADB/accessibility dumps, or another proxy to satisfy a criterion whose contract requires physical user interaction.

Fresh verifier sessions share durable device/repository/evidence state, not transcript context. Give each verifier exact acceptance criteria, allowed environment/device mutations and evidence destination (default `.flow/evidence/<head>/<capsule-id>/`, as an absolute path when the verifier runs in another checkout). It may operate the designated verification environment and capture evidence but does not edit production behavior or declare the unit accepted. When restore obligations exist, the receipt must report each target's `before` and `after` identity/hash plus MATCH/MISMATCH/UNKNOWN. A receipt may report `MATCH` only when the literal `before` and `after` values it renders use the same identity/hash scheme, are equal, and the supporting comparison confirms equality. If the rendered receipt conflicts with the proof or cannot self-consistently support `MATCH`, correct the receipt before yielding or report `UNKNOWN`; Main must not infer MATCH from a contradictory receipt. Main may reuse an unchanged MATCH receipt unless later mutation or contrary evidence makes it stale. Main consumes each compact receipt, independently inspects consequential evidence, and owns the acceptance decision.
