...
Fresh verifier sessions share durable device/repository/evidence state, not transcript context. Give each verifier exact acceptance criteria, allowed environment/device mutations and evidence destination. It may operate the designated verification environment and capture evidence but does not edit production behavior or declare the unit accepted. When restore obligations exist, the receipt must report each target's `before` and `after` identity/hash plus MATCH/MISMATCH/UNKNOWN. A receipt may report `MATCH` only when the literal `before` and `after` values it renders use the same identity/hash scheme, are equal, and the supporting comparison confirms equality. If the rendered receipt conflicts with the proof or cannot self-consistently support `MATCH`, correct the receipt before yielding or report `UNKNOWN`; Main must not infer MATCH from a contradictory receipt. Main may reuse an unchanged MATCH receipt unless later mutation or contrary evidence makes it stale. Main consumes each compact receipt, independently inspects consequential evidence, and owns the acceptance decision.
