...
33. LDD substantial planned work has two explicit user approval gates: approve the completed contract before planning dispatch, then approve the completed execution-grade plan before the first production-writing worker. Answers to open contract questions do not themselves approve the completed or materially amended contract unless the user explicitly says so. A unit-start/resume command never creates a missing approval; recorded unchanged approvals remain valid, and material contract/plan changes reopen their gate.
