...
33. LDD substantial planned work has two explicit user approval gates: approve the completed contract before planning dispatch, then approve the completed execution-grade plan before the first production-writing worker. A unit-start/resume command never creates a missing approval; recorded unchanged approvals remain valid, and material contract/plan changes reopen their gate.
34. Non-blocking orchestration does not permit speculative downstream phases: required acceptance/closure review and corrections that can change an evidence surface must settle before expensive device/manual/external evidence capture begins.
...
36. Interactive Main child/job completion waits are mechanically blocked by the Flow runtime guard: bare/job `hub wait` cannot hold the foreground; named process waits and targeted peer-reply waits remain available.
