...
15. Controllers never wait merely to observe a running worker. Child completion/yield is event-driven; Main records the pending dependency and remains interactive. Bounded waits are exceptional, only when an autonomous next immediate action strictly requires the result; progress polling is never the fallback.
...
27. Execution-grade task briefs are independently provable behavioral slices: split multiple separable Red→Green clusters into fresh executor capsules, but never impose a numeric file/turn/token quota on an inseparable behavior.
...
29. Before long/manual/device/external acceptance gates, durable work records a recovery checkpoint with exact head/tree, dirty/user-owned state, fresh evidence, remaining criteria, environment state and the exact next action.
30. Visual/device evidence collection is a bounded verification concern: prefer a fresh `@vision` verifier that may operate the designated environment and capture evidence but never edits production behavior or owns the acceptance decision.
31. Main/controller context is phase-scoped. After a durable phase boundary, prefer a fresh controller session when prior transcript context no longer contributes material judgment; rotation never hides unresolved state or creates a new approval gate.
