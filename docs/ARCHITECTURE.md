...
Interactive Flow also keeps a **forward pointer**: at meaningful user-facing checkpoints Main states the outcome, the next workflow action, and whether user input is required. Internal authorized next actions continue automatically; concrete questions are reserved for real design/approval/integration gates. Before the first device/emulator/manual/external acceptance action, that pointer becomes a durable recovery checkpoint (`flow-execution` section 9) so provider/session failure is a safe interruption. Acceptance is a stability barrier (`flow-evidence` rule): after Main accepts an independent acceptance/closure receipt, `flow_gate` binds that state to the current repository fingerprint; a later mutation makes the binding stale and verifier dispatch fails closed until the required closure/freshness basis is accepted and recorded again. Multi-step device/manual acceptance is verifier-owned when `flow-evidence-verifier` is available: each task carries both the governance scope and a mechanically checked capsule manifest, `flow-evidence-verifier` alone defines and enforces capsule independence and the restore receipt contract, and Main independently inspects consequential evidence and owns the judgment.
