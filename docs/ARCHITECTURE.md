...
LDD has two explicit human authorization gates around substantial planned work: approve the completed WHAT/contract before dispatching the planner, then approve the completed execution-grade HOW/plan before the first production-writing worker. Answers to open contract questions do not themselves approve the completed or materially amended contract unless the user explicitly says so. A generic unit-start/resume command cannot create a missing approval. Recorded approvals survive resume while their approved artifact/scope is materially unchanged; material changes reopen the corresponding gate. Plan approval authorizes local implementation within that envelope, not publication.
